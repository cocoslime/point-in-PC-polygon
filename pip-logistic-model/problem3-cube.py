...
test_xy_data = make_decode_CSV_list([DATA_DIR + "test/raster_10.csv"], record_defaults)
train_xy_data = make_decode_CSV_list([DATA_DIR + "train/raster_10.csv"], record_defaults)
...
batch_test_x, batch_test_y = tf.train.shuffle_batch([test_x_data, test_y_data],
                                                       min_after_dequeue=MIN_AFTER_DEQUEUE, capacity=CAPACITY, enqueue_many=False,
                                                       batch_size=BATCH_SIZE, num_threads=8)
# batch_test_x, batch_test_y = tf.train.batch([test_x_data, test_y_data], enqueue_many=True, batch_size=BATCH_SIZE, num_threads=8)
...
batch_train_x, batch_train_y, = tf.train.shuffle_batch([train_x_data, train_y_data],
                                                       min_after_dequeue=MIN_AFTER_DEQUEUE, capacity=CAPACITY, enqueue_many=False,
                                                       batch_size=BATCH_SIZE, num_threads=8)
# batch_train_x, batch_train_y = tf.train.batch([train_x_data, train_y_data], enqueue_many=True, batch_size=BATCH_SIZE, num_threads=8)
...
'''
Tensor("Conv3D:0", shape=(?, 10, 10, 10, 32), dtype=float32)
Tensor("MaxPool3D:0", shape=(?, 5, 5, 5, 32), dtype=float32)
'''
...
'''
Tensor("Conv3D_1:0", shape=(?, 5, 5, 5, 64), dtype=float32)
Tensor("Relu_1:0", shape=(?, 5, 5, 5, 64), dtype=float32)
Tensor("MaxPool3D_1:0", shape=(?, 3, 3, 3, 64), dtype=float32)
Tensor("dropout_1/mul:0", shape=(?, 3, 3, 3, 64), dtype=float32)
'''
...
L3_flat = tf.reshape(L3, [-1, 3 * 3 * 3 * 128])
...
with tf.Session() as sess:
    ...
    saver.restore(sess, "../tmp/problem3-10/model.ckpt")
