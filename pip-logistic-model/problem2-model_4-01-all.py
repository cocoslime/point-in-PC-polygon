...
# load_raster
def load_raster_data_in_array(file_path, width, height):
    x_data = []
    y_data = []
    convex_rate = []
    num_sides = []
...
test_x_data, test_y_data, test_convex_rate, test_num_sides = \
        load_raster_data_in_array(DATA_DIR + "/test.csv", WIDTH_NUM, HEIGHT_NUM)
...
with tf.Session() as sess:
    ...
    print("\nAccuracy: ", a)
    now = time.time()
    print("\n\nTime : ", int((now - start_time) / 60), "m ",  int(now - start_time), "s")
    # write result file
