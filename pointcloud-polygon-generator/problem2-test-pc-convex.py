...
def generate(raster_writer, vector_writer, reader, number):
    ...
    while i < number:
        ...
        for target, label in zip(tp_list, labels):
            ...
            '''
            raster data
            data_index, side_num, [pixel_data], label
            '''
            flat_list = [data_index, convex_ratio]
            ...
            '''
            vector data
            data_index, side_num, [target_point], [polygon_coords], label
            '''
            vector_data = [str(data_index), convex_ratio, target.x, target.y]
...
raster_test_file = open(RASTER_DIR + "convex_ratio/" + "test_" + '{0:03d}'.format(BUFFER) + ".csv", 'w', encoding='utf-8', newline='')
raster_test_writer = csv.writer(raster_test_file)

vector_test_file = open(VECTOR_DIR + "convex_ratio/" + "test_" + '{0:03d}'.format(BUFFER) + ".csv", 'w', encoding='utf-8', newline='')
