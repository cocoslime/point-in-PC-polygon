...
def generate(raster_writer, vector_writer, reader, number):
    ...
    while i < number:
        ...
        for target, label in zip(tp_list, labels):
            ...
            '''
            vector_data = [str(data_index), side_num, target.x, target.y]
...
raster_test_file = open(RASTER_DIR + "side_num/" + "test_" + '{0:03d}'.format(BUFFER) + ".csv", 'w', encoding='utf-8', newline='')
raster_test_writer = csv.writer(raster_test_file)

vector_test_file = open(VECTOR_DIR + "side_num/" + "test_" + '{0:03d}'.format(BUFFER) + ".csv", 'w', encoding='utf-8', newline='')
